...
func (h *ConnectHandler) checkDBConnection() error {
	psqlconn := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable", h.dbSettings.Host, h.dbSettings.Port, h.dbSettings.User, h.dbSettings.Password, h.dbSettings.Name)
